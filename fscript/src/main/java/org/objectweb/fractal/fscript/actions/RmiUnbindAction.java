...
import org.objectweb.fractal.fscript.reconfiguration.RmiUnbindReconfiguration;
import org.objectweb.fractal.rmi.registry.NamingService;

public class RmiUnbindAction extends AbstractProcedure {
    ...
    public Object apply(Object[] args, Environment env) throws FScriptException {
        ...
        if (ns == null) {
            throw new ScriptExecutionError(Diagnostic
                    .error("rmi_unbind: not connected to a Fractal RMI registry."));
        } else {
            String name = typedArgument(args[0], String.class);
            return new RmiUnbindReconfiguration(ns, name).apply();
        }
    }
