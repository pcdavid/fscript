...
/**
 * Wraps <code>BindingController#bindFc(itfName, serverItf)</code> into an atomic
 * action.
 * 
...
public class BindReconfiguration extends AbstractReconfiguration {
    ...
    public BindReconfiguration(Component client, String clientItfName, Interface serverItf) {
        assert (client != null);
        assert (clientItfName != null && clientItfName.length() > 0);
        assert (serverItf != null);
        this.client = client;
    ...
    protected Object apply(Transaction tx) throws NoSuchInterfaceException,
        ...
        if (prevItf == null) {
            ...
            tx.registerUndo(new UnbindReconfiguration(client, clientItfName));
        } else {
            throw new ReconfigurationFailedException(
                    "Client interface already bound to another server interface.");
        }
